...
public class HaveStatusEffectGoalBuilder<T> extends GoalBuilder<ServerPlayer, T> {
    public HaveStatusEffectGoalBuilder(GoalOptionSupplier<T> optionSupplier, GoalProgressSupplier<T, List<MobEffectInstance>, ?> progressSupplier) {
        super("EFFECT", "Get", GoalCategory.STATUS_EFFECTS, optionSupplier, progressSupplier.map(p -> p.getActiveEffects().stream().toList()));
    }
